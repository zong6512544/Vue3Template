...
export class Live2dService {
  ...
  formatEveryData({ series, person, currPerson }): object {
    const {
      ...
      textures = [],
      expressions = [],
      motions: { flick_head = [], idle = [], sleepy = [] } = {},
    } = currPerson;

    const config = { ...currPerson };
    ...
    // 格式化expressions
    expressions &&
      (config.expressions = this.formatFile(expressions, `${basePrefix}/`));

    // motions:
    // motions-flick_head （not）
    flick_head.length &&
      (config.motions.flick_head = this.formatFile(
        flick_head,
        `${basePrefix}/`,
      ));
    // motions-idle
